...
const CheckoutPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (result.success) {
                alert('سفارش شما با موفقیت ثبت شد! (عملیات شبیه‌سازی شده)');
                navigate(`/order-success/${result.order.order_number}`);
            } else {
                alert(result.error || 'خطا در ثبت سفارش (شبیه‌سازی شده)');
            }
        } catch (error) {
            console.error('Checkout error (mocked):', error);
            alert('خطا در ثبت سفارش (شبیه‌سازی شده)');
        } finally {
            setLoading(false);
        }
    };
